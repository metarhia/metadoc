...
const splitComments = lines => {
  ...
  if (hasSignature) {
    i++;
    while (i < lines.length && lines[i].startsWith(FOUR_SPACES)) i++;
  }
  const paramStart = i;
...
const parseFunction = (fn, lines) => {
  ...
  const sig = {
    ...
    isGetter: fnStr.startsWith('get'),
    isSetter: fnStr.startsWith('set'),
    argsSignature: '',
    parameters: [],
    nativeParams: '',
    comments: [],
  ...
  if (typeof fn === 'function') {
    ...
    const nativeParams = fnStr.match(nativeParamRegex);
    if (nativeParams !== null) sig.nativeParams = nativeParams[0].slice(1, -1);
  }
      ...
      if (fn.prototype) {
        ...
        for (const prop of props) {
          ...
          if (prop === 'constructor') {
            ...
            const propFn = desc.value || desc.get || desc.set;
            if (typeof propFn === 'function') {
              entities.set(propName, parseSignature(propFn, text, start));
            }
          }
        }
      }
