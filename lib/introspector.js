...
const findMatchingBrace = (str, startBrace, endBrace, startBraceIndex) => {
  let end = (startBraceIndex || str.indexOf(startBrace)) + 1;
  for (let count = 1; end < str.length; end++) {
    const ch = str[end];
    if (ch === startBrace) count++;
    else if (ch === endBrace) {
      count--;
      if (count === 0) break;
    }
...
const separateCommentTypes = comment => {
  ...
  while (true) {
    ...
    let end;
    if (comment[0] === '<') {
      end = findMatchingBrace(comment, '<', '>', start);
    } else if (comment[0] === '[') {
      end = findMatchingBrace(comment, '[', ']', start);
    } else {
      break;
