...
const UNDESCORE_REGEXP = /_/g;

const escapeUndescore = str => str.replace(UNDESCORE_REGEXP, '\\_');
...
const generateProperties = (method, comments, customTypes, usedTypes) => {
  ...
  for (const param of comment.parameters) {
    ...
    buf.push(escapeUndescore(`#### ${method}.${param.name}\n`));
  ...
  for (const [name, methods] of inventory) {
    ...
    for (const [method, signature] of methods) {
      ...
      if (signature.isClass) {
        buf.push(escapeUndescore(`#### class ${method}\n`));
      } else {
        buf.push(
          escapeUndescore(`#### ${asyncPrefix}${method}(${args.join(', ')})\n`)
        );
      }
      buf.push(
        ...generateParameters(
