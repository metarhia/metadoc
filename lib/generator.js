...
const generateMd = (inventory, options = {}) => {
  ...
  for (const [name, methods] of inventory) {
    ...
    for (const [method, signature] of methods) {
      ...
      let prefix = '';
      if (signature.isAsync) prefix = 'async ';
      else if (signature.isGetter) prefix = 'get ';
      else if (signature.isSetter) prefix = 'set ';

      if (signature.isClass) {
        buf.push(headerify(`class ${method}\n`, level + 1));
      } else if (method.indexOf('.') === -1) {
        buf.push(
          headerify(`${prefix}${method}(${args.join(', ')})\n`, level + 1)
        );
      } else {
        buf.push(
          headerify(`${prefix}${method}(${args.join(', ')})\n`, level + 2)
