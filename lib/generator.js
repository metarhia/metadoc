...
const wrapComment = (comment, customTypes, usedTypes) =>
  comment.trimLeft()
    .replace(
      /<[\w.]+(\[])?>/g,
      type => wrapType(type.slice(1, -1), customTypes, usedTypes)
    );
...
const generateProperties = (method, comments, customTypes, usedTypes) => {
  ...
  for (const param of comment.parameters) {
    ...
    line = formatLine(line, 0, COMMENTS_SECONDARY_OFFSET);
    buf.push(line, '');
  }
...
const generateRest = (comments, customTypes, usedTypes) => {
  ...
  for (const comment of comments) {
    ...
    if (comment.name === 'Example' || comment.name === 'Result') {
      const code = comment.comment.trimLeft();
    ...
    for (const [method, signature] of methods) {
      ...
      if (signature.argsSignature) {
        ...
        if (checkArgsSignature(signatureArgs, args)) {
          ...
          console.error(`Arguments signature for function ${method} ` +
      ...
      if (signature.title) {
        buf.push(formatLine(wrapComment(
          signature.title, options.customTypes, usedTypes
        ), 0, COMMENTS_SECONDARY_OFFSET), '');
      }
      if (signature.description) {
        buf.push(formatLine(wrapComment(
          signature.description, options.customTypes, usedTypes
        ), 0, COMMENTS_SECONDARY_OFFSET), '');
      }
      buf.push(...generateRest(
        signature.comments, options.customTypes, usedTypes
      ));
      buf.push(...generateProperties(
        method, signature.comments, options.customTypes, usedTypes
      ));
    }
  }
