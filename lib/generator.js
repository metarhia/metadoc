...
const genClassSig = (name, sig, opts) => {
  ...
  opts.customLinks.push([opts.customClasses.get(name), genClassLink(header)]);
  return [header, ''];
};
...
const getSignatures = (name, sig) => {
  ...
  if (sig.argsSignature.length) {
    ...
    signatures = sig.argsSignature.map(s => {
      ...
      compliant = compliant || checkArgsCompliance(argsSignature, defaultArgs);
...
const generateSignatures = (name, sig, opts, methods) => {
  ...
  return signatures.map(args =>
    headerify(`${asyncPrefix}${name}(${args.join(', ')})\n`, level)
  );
};
...
const generateProperties = (method, comments, opts) => {
  ...
  for (const param of comment.parameters) {
    ...
    if (param.comment) line += ' ' + wrapComment(param.comment, opts);
    buf.push(
      headerify(`${method}.prototype.${param.name}\n`, opts.propertyLevel),
      formatLine(line, 0),
      ''
    );
  }
...
const generateMd = (inventory, options) => {
  ...
  const buf = [];
  if (opts.header) buf.push(opts.header);
  ...
  for (const [name, methods] of inventory) {
    ...
    for (const [method, signature] of methods) {
      try {
        buf.push(...generateFunction(method, signature, opts, methods));
      } catch (err) {
